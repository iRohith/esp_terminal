...
/// Implements a connector for WebSocket connections to local services.
///
/// This class extends [BaseConnector] and enables communication with a local
/// server over WebSockets for local network data exchange or control.
class WSLocalConnector extends BaseConnector {
  /// Observable string storing the WebSocket URL for local connection.
  ///
  /// Retrieves the URL from [DataService] or defaults to an empty "ws://" string.
  final wsurl = DataService.to.get("websocket_url", () => "ws://".obs);
  ...
  /// Initializes the WebSocket local connector.
  ///
  ...
  /// Establishes a connection to the WebSocket local service.
  ///
  /// Presents a dialog for the user to confirm or edit the WebSocket URL.
  /// Upon confirmation, attempts to connect to the specified URL.
  ...
  Future<bool> connect() async {
    try {
      ...
      // The `connected.value` will be updated based on the dialog's result.
      connected.value =
          true ==
          await Get.dialog(
            AlertDialog(
  ...
  /// Disconnects from the currently connected WebSocket local service.
  ///
  ...
  /// Provides a stream for reading incoming data from the WebSocket local service.
  ///
  /// Listens to the WebSocket's event stream, processing binary data as raw bytes
  /// and intentionally ignoring text data (as this connector is for binary communication).
  ...
  Stream<int> read() {
    ...
    showSnackbar("Connected", "Successfully connected to WebSocket local.");
        ...
        .map<Uint8List>((e) {
          ...
          switch (e) {
            case TextDataReceived():
              // Text data is received but intentionally ignored for this connector,
              // as this local WebSocket is designed for binary communication.
              break;
          ...
          return empty; // For any other event type or ignored text data, return an empty list.
        })
        .expand(
          (e) => e,
  ...
  /// Writes a buffer of data to the WebSocket local service.
  ///
